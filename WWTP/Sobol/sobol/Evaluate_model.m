...
fprintf(fileRes,'%s\n',htg{:});
fclose(fileRes);
filename_res = ('C:\osmose_julia\ET\pulp\Sobol\sobol\Sobol_results.txt'); 
...
if not(isfile(filename))  % run osmose 
...
if not(isfile(filename)) % if osmsose ru did not succeed, use old results 
